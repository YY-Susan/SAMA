...
class EOD_Preprocessor:
    ...
    def generate_feature(self, selected_tickers_fname, begin_date, opath,
        ...
        for stock_index, single_EOD in enumerate(self.data_EOD):
            ...
            open_max = np.max(selected_EOD[begin_date_row:, 1])
            high_max = np.max(selected_EOD[begin_date_row:, 2])
            low_max = np.max(selected_EOD[begin_date_row:, 3])
            ...
            for row in range(begin_date_row, selected_EOD.shape[0]):
                ...
                if cur_index - int(selected_EOD[row - return_days][0]) == \
                    ...
                    features[cur_index - pad_begin][5] = \
                        selected_EOD[row][1] / open_max
                    features[cur_index - pad_begin][6] = \
                        selected_EOD[row][2] / high_max
                    features[cur_index - pad_begin][7] = \
                        selected_EOD[row][3] / low_max
                    features[cur_index - pad_begin][8] = \
                        selected_EOD[row][5] / volume_max

            for i in range(len(features)):
                for j in range(1, features.shape[1]):
                    if (features[i][j] + 1234) < 1e-8:
                        d = i
                        while d >= 0 and (features[d][j] + 1234) < 1e-8:
                            d -= 1
                        u = i
                        while u < len(features) and (features[u][j] + 1234) < 1e-8:
                            u += 1

                        if d >= 0 and u < len(features):
                            features[i][j] = (features[d][j]+features[u][j]) / 2
                        elif d < 0 and u < len(features):
                            features[i][j] = features[u][j]
                        elif d >= 0 and u >= len(features):
                            features[i][j] = features[d][j]
